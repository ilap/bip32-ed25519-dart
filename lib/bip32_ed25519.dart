...
class CardanoExtendedPrivateKey extends ByteList
  ...
  SignedMessage sign(List<int> message) {
    ...
    final result = TweetNaCl.crypto_sign(
        sm, -1, Uint8List.fromList(message), 0, message.length, Uint8List.fromList(this + verifyKey), true);
    if (result != 0) {
...
class CardanoBip32PublicKey extends CardanoExtendedPublicKey implements ExtendedPublicKey {
...
class CardanoBip32PrivateKey extends CardanoExtendedPrivateKey implements ExtendedPrivateKey {
  ExtendedKey get public =>  this.publicKey;
  ByteList get chainCode => suffic;
  ExtendedKey derive(int index);
}
